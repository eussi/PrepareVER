...
Shell scripts
### Function:
Through mysql,sshpass command, query the transaction logs generated by different servers on one server
### Usage:
1. logs:  
	&#8194;&#8194;Query the time transaction log through this script
2. logs.ini:  
	&#8194;&#8194;Logs configuration file, where information about each environment is configured
### Notice:
- Note the address configuration of the configuration file, which is included in the logs script
- This script depend on the mysql and sshpass commands

## fort
### type:
Shell scripts
### Function:
Fortress machine login procedure
### Usage:
1. fort:  
	&#8194;&#8194;Run this script to select the server to log on to, and the server information is from the configuration file hosts.ini
2. hosts.ini:  
	&#8194;&#8194;Configure server information for each environment, and each server can contain more than one user information
### Notice:
- Note the format of the configuration, referring to the configured content
- This script depend on the sshpass commands